...
description: |
  ...
  输出包含精美的 Markdown 长文报告和可打印的 PDF。报告风格为故事化叙述，以第二人称（"你"）
  视角深入解读用户11年音乐收藏背后的灵魂画像。
...
将用户的网易云"我喜欢的音乐"歌单，转化为一份深度个性化的音乐品味编年史报告（Markdown + PDF）。
...
如果playwright不可用，告知用户可以直接用浏览器打开HTML文件并打印为PDF。
